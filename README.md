...
**Local deliverable, not a completed deployment.** No GitHub repository, DNS records, domain ownership or live deployment is established by these files. `mainsource.me` is the proposed default, not a verified domain. The contact mailbox and legal details must be verified before public launch.
...
The tests read current public contact/legal values from the generated `dist/site-build.json`, so those configuration changes do not require hard-coded test edits. If you deliberately change page titles, headings, section IDs or email subject wording, update the corresponding content expectations in `tests/site-fixtures.json`. The manifest contains only already-public website configuration; never add secrets to `src/config.ts`.
...
src/pages/Legal.tsx           Privacy, terms and data-deletion drafts
...
- `dist/data-deletion/index.html` → `/data-deletion/`
...
| Value | What to set |
| --- | --- |
| `company.email` | An active, monitored mailbox. **`hello@mainsource.me` is a placeholder and must be confirmed or replaced.** This updates contact and policy email links. |
| `company.linkedInUrl` | The actual full company-profile URL. An empty string displays **LinkedIn (coming soon)** as text, not a dead link. |
...
| `company.location` | The accurate public location. |
| `company.legal` | The real legal entity name, registered address, company registration number and VAT/tax information, as applicable and professionally reviewed. |
...
### Legal content is deliberately unfinished

`src/pages/Legal.tsx` contains clearly labeled **draft** policies and legal-detail placeholders. They are not legal advice or a compliance certification.

Before publishing:

1. Complete `company.legal` with genuine, verified information, and confirm the mailbox works.
2. Have a qualified legal professional review all three policies against the actual entity, jurisdiction, hosting, correspondence handling and retention practices.
3. Apply the approved wording, including any required dates or disclosures.
4. **Only after that review and verification**, remove or replace the draft notice in `src/pages/Legal.tsx`, update its other “pending/placeholder” wording, and remove the **“details pending”** label in `src/components/Footer.tsx`.
5. Keep the final operator details visible and accurate; do not merely hide the warning while leaving placeholders behind.
...
| Variable | Meaning |
| --- | --- |
| `SITE_URL` | Public origin only, e.g. `https://OWNER.github.io` or `https://example.com`. No repository path, query, fragment or credentials. Local default: `https://mainsource.me` (proposed, unverified). Use HTTPS for production. |
...
Also visit `/REPOSITORY/privacy/`, `/REPOSITORY/terms/`, `/REPOSITORY/data-deletion/` and a deliberately missing URL. Restore your intended variables and rebuild afterward.
...
2. Create a GitHub repository and upload/push this project. This supplied directory is not initialized as a Git repository; no repository has been created for you.
...
If you want to initialize and push this directory yourself, first create an **empty** GitHub repository, replace the remote below, and run:

```sh
git init -b main
git add .
git commit -m "Add MainSource website"
git remote add origin git@github.com:OWNER/REPOSITORY.git
git push -u origin main
```

These are optional instructions, not actions already performed. Review staged files before committing; do not commit credentials. `node_modules/`, `.env`, `dist/` and temporary/test output are ignored. Commit the source, original logo, workflow, manifest and lockfile.

### Custom domain and HTTPS

Use only a domain you control. If choosing `mainsource.me`, verify ownership and availability independently; its presence in this project is not evidence of either.
...
- [ ] Set a genuine LinkedIn URL or keep the explicit non-link “coming soon” state; enable WhatsApp only for an approved working number.
- [ ] Replace every legal placeholder, obtain professional policy review, and only then update/remove draft and “details pending” wording.
...
- [ ] Test direct requests and refreshes for all legal pages, plus a missing URL returning the custom 404 instead of the homepage.
...
Keep unreviewed drafts local. Successful builds/tests do not verify domain ownership, email delivery, legal compliance or real-world performance.
